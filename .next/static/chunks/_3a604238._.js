...
"[project]/component/splashcursor.tsx [app-client] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
function Splashcursor({ SIM_RESOLUTION = 128, DYE_RESOLUTION = 512, CAPTURE_RESOLUTION = 512, DENSITY_DISSIPATION = 3.5, VELOCITY_DISSIPATION = 2, PRESSURE = 0.1, PRESSURE_ITERATIONS = 20, CURL = 3, SPLAT_RADIUS = 0.2, SPLAT_FORCE = 6000, SHADING = true, COLOR_UPDATE_SPEED = 10, BACK_COLOR = {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "Splashcursor.useEffect": ()=>{
            ...
            let adjustedDyeResolution = DYE_RESOLUTION;
            const isHighEndDevice = window.devicePixelRatio <= 2 && navigator.hardwareConcurrency >= 4;
            if (DYE_RESOLUTION > 1024 && !isHighEndDevice) {
                console.warn(`Reducing DYE_RESOLUTION from ${DYE_RESOLUTION} to 1024 for performance`);
                adjustedDyeResolution = 1024;
            }
            ...
            let config = {
                SIM_RESOLUTION,
                DYE_RESOLUTION: adjustedDyeResolution,
                CAPTURE_RESOLUTION,
                DENSITY_DISSIPATION,
            ...
            if (!ext.supportLinearFiltering) {
                ...
                console.warn('Linear filtering not supported; reduced DYE_RESOLUTION to 256');
            }
            ...
            const isWebGL2 = gl instanceof WebGL2RenderingContext;
            const baseVertexShader = compileShader(gl.VERTEX_SHADER, isWebGL2 ? `
          #version 300 es
        ...
        `);
            const copyShader = compileShader(gl.FRAGMENT_SHADER, isWebGL2 ? `
          #version 300 es
        ...
        `);
            const clearShader = compileShader(gl.FRAGMENT_SHADER, isWebGL2 ? `
          #version 300 es
          precision mediump float;
          in highp vec2 vUv;
          uniform sampler2D uTexture;
        ...
        `);
            const displayShaderSource = isWebGL2 ? `
        #version 300 es
        precision highp float;
        in vec2 vUv;
        in vec2 vL;
      ...
      `;
            const splatShader = compileShader(gl.FRAGMENT_SHADER, isWebGL2 ? `
          #version 300 es
          precision highp float;
          in vec2 vUv;
        ...
        `);
            const advectionShader = compileShader(gl.FRAGMENT_SHADER, isWebGL2 ? `
          #version 300 es
          precision highp float;
          in vec2 vUv;
        ...
        `, ext.supportLinearFiltering ? null : [
            ...
            const divergenceShader = compileShader(gl.FRAGMENT_SHADER, isWebGL2 ? `
          #version 300 es
          precision mediump float;
          in highp vec2 vUv;
          in highp vec2 vL;
        ...
        `);
            const curlShader = compileShader(gl.FRAGMENT_SHADER, isWebGL2 ? `
          #version 300 es
          precision mediump float;
          in highp vec2 vUv;
          in highp vec2 vL;
        ...
        `);
            const vorticityShader = compileShader(gl.FRAGMENT_SHADER, isWebGL2 ? `
          #version 300 es
          precision highp float;
          in vec2 vUv;
          in vec2 vL;
        ...
        `);
            const pressureShader = compileShader(gl.FRAGMENT_SHADER, isWebGL2 ? `
          #version 300 es
          precision mediump float;
          in highp vec2 vUv;
          in highp vec2 vL;
        ...
        `);
            const gradientSubtractShader = compileShader(gl.FRAGMENT_SHADER, isWebGL2 ? `
          #version 300 es
          precision mediump float;
          in highp vec2 vUv;
          in highp vec2 vL;
        ...
        `);
            ...
            const handleMouseDown = {
                "Splashcursor.useEffect.handleMouseDown": (e)=>{
            ...
            }["Splashcursor.useEffect.handleMouseDown"];
            const handleFirstMouseMove = {
                "Splashcursor.useEffect.handleFirstMouseMove": (e)=>{
                    let pointer = pointers[0];
                    let posX = scaleByPixelRatio(e.clientX);
                    let posY = scaleByPixelRatio(e.clientY);
                    updateFrame();
            ...
            }["Splashcursor.useEffect.handleFirstMouseMove"];
            ...
            const handleTouchStart = {
                "Splashcursor.useEffect.handleTouchStart": (e)=>{
                    e.preventDefault();
            ...
            }["Splashcursor.useEffect.handleTouchStart"];
            const handleFirstTouchStart = {
                "Splashcursor.useEffect.handleFirstTouchStart": (e)=>{
                    e.preventDefault();
                    const touches = e.targetTouches;
                    let pointer = pointers[0];
                    for(let i = 0; i < touches.length; i++){
            ...
            }["Splashcursor.useEffect.handleFirstTouchStart"];
            ...
            const handleTouchEnd = {
                "Splashcursor.useEffect.handleTouchEnd": (e)=>{
            ...
            }["Splashcursor.useEffect.handleTouchEnd"];
            // Log HTML attributes for debugging
            console.log('Initial HTML attributes:', Object.fromEntries([
                ...document.documentElement.attributes
            ].map({
                "Splashcursor.useEffect": (attr)=>[
                        attr.name,
                        attr.value
                    ]
            }["Splashcursor.useEffect"])));
...
//# sourceMappingURL=_3a604238._.js.map